...
These tests validate the critical database functionality with minimal mocking:
...
Uses real database connections for better test fidelity and reduced overmocking.
...
class TestDatabaseSchemaSmoke:
    ...
    @pytest.mark.asyncio
    async def test_wal_mode_assertion(self, isolated_db: DatabaseTestManager):
    ...
    @pytest.mark.asyncio
    async def test_required_tables_exist(self, isolated_db: DatabaseTestManager):
    ...
    @pytest.mark.asyncio
    async def test_foreign_keys_enabled(self, isolated_db: DatabaseTestManager):
    ...
    @pytest.mark.asyncio
    async def test_schema_version_exists(self, patched_db_connection):
        """Test that schema version table exists and contains version 3."""
        # This test uses patched connection to test the global function
...
class TestErrorEnvelopeValidation:
    ...
    async def test_database_schema_error_structure(
        self, isolated_db: DatabaseTestManager
    ):
    ...
    @pytest.mark.asyncio
    async def test_health_check_error_envelope(self, patched_db_connection):
        """Test health check returns proper error envelope on failure."""
        # Test healthy case first with properly isolated database
...
class TestUtcTimestampValidation:
    ...
    @pytest.mark.asyncio
    async def test_utc_timestamp_creation(self, isolated_db: DatabaseTestManager):
        ...
        async with isolated_db.get_connection() as conn:
            await conn.execute(
                ...
                VALUES (?, ?, ?, ?)
                """,
                (session_id, purpose, created_by, now.isoformat()),
            )
            await conn.commit()
    ...
    @pytest.mark.asyncio
    async def test_metadata_round_trip(self, isolated_db: DatabaseTestManager):
...
# Removed old isolated_db fixture - replaced with isolated_db from fixtures.database
...
class TestDatabaseSchema:
    ...
    @pytest.mark.asyncio
    async def test_schema_initialization(self, isolated_db: DatabaseTestManager):
    ...
    @pytest.mark.asyncio
    async def test_schema_version_tracking(self, isolated_db: DatabaseTestManager):
    ...
    @pytest.mark.asyncio
    async def test_views_creation(self, isolated_db: DatabaseTestManager):
...
class TestPragmaSettings:
    ...
    @pytest.mark.asyncio
    async def test_pragma_application(self, isolated_db: DatabaseTestManager):
    ...
    @pytest.mark.asyncio
    async def test_multiple_connections_pragma_consistency(
        self, isolated_db: DatabaseTestManager
    ):
...
class TestForeignKeyEnforcement:
    ...
    @pytest.mark.asyncio
    async def test_foreign_key_enforcement_enabled(
        self, isolated_db: DatabaseTestManager
    ):
    ...
    @pytest.mark.asyncio
    async def test_cascade_delete_sessions(self, isolated_db: DatabaseTestManager):
...
class TestUpdatedTriggers:
    ...
    @pytest.mark.asyncio
    async def test_sessions_updated_trigger(self, isolated_db: DatabaseTestManager):
    ...
    @pytest.mark.asyncio
    async def test_agent_memory_updated_trigger(self, isolated_db: DatabaseTestManager):
...
class TestJsonValidation:
    ...
    @pytest.mark.asyncio
    async def test_valid_json_metadata_accepted(self, isolated_db: DatabaseTestManager):
    ...
    @pytest.mark.asyncio
    async def test_invalid_json_metadata_rejected(
        self, isolated_db: DatabaseTestManager
    ):
    ...
    @pytest.mark.asyncio
    async def test_null_metadata_allowed(self, isolated_db: DatabaseTestManager):
...
class TestIndexPerformance:
    ...
    @pytest.mark.asyncio
    async def test_indexes_exist(self, isolated_db: DatabaseTestManager):
    ...
    @pytest.mark.asyncio
    async def test_index_usage(self, isolated_db: DatabaseTestManager):
...
class TestUtcTimestamps:
    ...
    @pytest.mark.asyncio
    async def test_utc_timestamp_functions(self, isolated_db: DatabaseTestManager):
    ...
    @pytest.mark.asyncio
    async def test_database_timestamp_storage(self, isolated_db: DatabaseTestManager):
...
class TestHealthCheck:
    ...
    @pytest.mark.asyncio
    async def test_health_check_healthy(self, patched_db_connection):
    ...
    @pytest.mark.asyncio
    async def test_get_schema_version(self, patched_db_connection):
        """Test schema version retrieval with proper isolation."""
        version = await get_schema_version()
        assert version == 3
...
class TestCleanupOperations:
    ...
    @pytest.mark.asyncio
    async def test_cleanup_expired_data(self, isolated_db: DatabaseTestManager):
...
class TestConnectionPooling:
    ...
    @pytest.mark.asyncio
    async def test_connection_statistics(self, isolated_db: DatabaseTestManager):
        """Test connection statistics tracking."""

        initial_stats = isolated_db.get_stats()
        assert initial_stats["connection_count"] == 0
        assert initial_stats["is_initialized"] is True

        # Use a connection
        async with isolated_db.get_connection():
            # Connection count should be tracked during usage
            pass

        # Connection should be cleaned up after context exit
        final_stats = isolated_db.get_stats()
        assert final_stats["connection_count"] == 0

    @pytest.mark.asyncio
    async def test_concurrent_connections(self, isolated_db: DatabaseTestManager):
...
@pytest.mark.asyncio
async def test_integration_with_global_functions(patched_db_connection):
    """Test integration with global database functions using isolated database."""

    # Test execute_query with patched database
    results = await execute_query("SELECT name FROM sqlite_master WHERE type='table'")
