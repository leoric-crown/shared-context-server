...
test: ## Run tests with coverage
	...
	@start=$$(date +%s); \
	if USE_SQLALCHEMY=true uv run pytest --cov=src --cov-report=term-missing --cov-report=html --cov-report=xml --timeout=30; then \
		end=$$(date +%s); \
		duration=$$((end - start)); \
...
test-quick: ## Run tests without coverage (faster)
	...
	@start=$$(date +%s); \
	if uv run pytest -x --tb=short --timeout=30; then \
		end=$$(date +%s); \
		duration=$$((end - start)); \
...
test-backend: ## Test both database backends
	...
	if USE_SQLALCHEMY=false uv run pytest -q --maxfail=3 --timeout=30; then \
		aio_status="✅"; \
	fi; \
	echo "  → Testing SQLAlchemy backend..."; \
	if USE_SQLALCHEMY=true uv run pytest -q --maxfail=3 --timeout=30; then \
		sql_status="✅"; \
	fi; \
	end=$$(date +%s); \
	duration=$$((end - start)); \
